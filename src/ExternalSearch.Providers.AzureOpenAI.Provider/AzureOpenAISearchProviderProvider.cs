...
using CluedIn.ExternalSearch.Providers.AzureOpenAI;
...
public class AzureOpenAISearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
{
    public AzureOpenAISearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(
        ...
        IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
    {
        ...
        var result = new AzureOpenAIExternalSearchJobData(configuration);
    ...
    {
        if (jobData is AzureOpenAIExternalSearchJobData result)
        {
            return await Task.FromResult(result.ToDictionary());
        }

        throw new InvalidOperationException(
            $"Unexpected data type for {nameof(AzureOpenAIExternalSearchJobData)}, {jobData.GetType()}");
    }
